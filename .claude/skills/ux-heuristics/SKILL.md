...
> **Project note (Event Attendance System):** the sibling skills this file references
> (`refactoring-ui`, `cro-methodology`) are **not installed** in this repo — they're
> visual-polish and conversion-optimization tools from the same upstream collection,
> irrelevant to an internal operator tool with no visitors to convert. Use this skill
> for its Krug/Nielsen usability review and `references/dark-patterns.md`; the
> `references/wcag-checklist.md` bundled here duplicates (independently) the
> `reviewing-a11y` skill's own WCAG reference — that's expected, not an error, since
> the two skills serve different primary purposes (general usability vs formal
> accessibility conformance) and each is meant to stand alone. See
> `docs/claude-skills.md` for how these two skills' scopes differ. For this app
> specifically, weight `references/dark-patterns.md`'s "confirmation" and
> "obstruction" patterns highly — this app is an event-day operator tool where a
> confusing workflow can mean lost or duplicated attendance, not a lost sale.
